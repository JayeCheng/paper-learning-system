...
def run_daily_pipeline(date: str | None = None, *, root: Path = Path(".")) -> tuple[DailyReport, Path, Path]:
    ...
    candidates.extend(load_classic_fallback_candidates(max_daily, existing_count=len(candidates)))
    normalized = normalize_papers(candidates)
    unique_papers = dedupe_papers(normalized)
